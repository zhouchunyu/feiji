...
var continueDiv=document.getElementById("continuediv");
    //找人帮忙，分享好友
var shareFriendDiv=document.getElementById("sharefrienddiv");
    //弹出遮罩层
var shadowModalDiv=document.getElementById("shadow_modal");    

    //获得掉落人物区域
var downmanDiv=document.getElementById("downman");


    //随机产生5个文案
window.onload = choosePic;
function choosePic() {
    var myPix = new Array("image/zhao1.png","image/zhao2.png","image/zhao3.png","image/zhao4.png","image/zhao5.png");
...
移动行为
    ...
    this.init=function(){
        ...
        downmanDiv.appendChild(this.imagenode);
    }
    this.init();
}
...
function oddbullet(X,Y){
    bullet.call(this,X,Y,6,14,"image/hongbao.png");
}
...
function ourplan(X,Y){
    var imagesrc="image/66-80.png";
    // var imagesrc="image/1-6.png";
    // plan.call(this,1,X,Y,66,80,0,660,0,"image/本方飞机爆炸.gif",imagesrc);
    plan.call(this,1,X,Y,66,80,0,660,0,"image/66-82.gif",imagesrc);  
...
var selfplan=new ourplan(w/2-27,485);
...
var bianjie=function(){
    ...
    if(bodyobjX<0||bodyobjX>w||bodyobjY<0||bodyobjY>h){
        if(document.removeEventListener){
            mainDiv.removeEventListener("touchmove",yidong,true);
        }
        else if(document.detachEvent){
...
function start(){
     ...
     */
    //随机产生5个中等下落的人物    
    var myPixDownImg = new Array("image/mid1.png","image/mid2.png","image/mid3.png","image/mid4.png","image/mid5.png");
    var randomNum = Math.floor((Math.random() * myPixDownImg.length));
    var downImgSrc=myPixDownImg[randomNum];
    if(mark==20){
        mark1++;
        //中飞机
        if(mark1%5==0){
            enemys.push(new enemy(6,25,274,46,60,5000,360,random(1,3),"image/38-34.gif",downImgSrc));
        }
        //大飞机
        if(mark1==20){
            enemys.push(new enemy(12,57,210,110,164,30000,540,1,"image/38-34.gif",downImgSrc));
            mark1=0;
        }
        //小飞机
        else{
            enemys.push(new enemy(1,19,286,34,24,1000,360,random(1,4),"image/38-34.gif",downImgSrc));
        }
        mark=0;
    }
 ...
 */
        if(enemys[i].imagenode.offsetTop>568){
            ...
            downmanDiv.removeChild(enemys[i].imagenode);
            enemys.splice(i,1);
            enemyslen--;
        }
            ...
            if(enemys[j].planisdie==false){
                if(enemys[j].imagenode.offsetLeft+enemys[j].plansizeX>=selfplan.imagenode.offsetLeft&&enemys[j].imagenode.offsetLeft<=selfplan.imagenode.offsetLeft+selfplan.plansizeX){
                  if(enemys[j].imagenode.offsetTop+enemys[j].plansizeY>=selfplan.imagenode.offsetTop+40&&enemys[j].imagenode.offsetTop<=selfplan.imagenode.offsetTop-20+selfplan.plansizeY){
                    ...
                    //   selfplan.imagenode.src="image/66-82.gif";
                      selfplan.imagenode.src="image/zhutou.gif";
...
//本方猪挂后弹出文案，文案关闭后几种选择情况1、领取优惠券进商场 2、继续游戏 3、找人帮忙（分享）
function choseGameMethod(){
    startdiv.style.display="none";
...
function shareFriend(){
    ...
    shareFriendDiv.style.display="block";
}
/*
    完成界面的初始化
    敌方小飞机一个
